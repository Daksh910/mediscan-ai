export const DNAHelix = ({ className = '' }: { className?: string }) => (
  <svg viewBox="0 0 80 200" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
    {Array.from({ length: 10 }).map((_, i) => {
      ...
      return (
        <g key={i}>
          <circle cx={40 + offset} cy={y} r="4" fill="hsl(158 42% 30%)" opacity={0.6 + Math.sin(i) * 0.3}>
            <animate attributeName="cx" values={`${40+offset};${40-offset};${40+offset}`} dur="3s" repeatCount="indefinite" />
          </circle>
          <circle cx={40 - offset} cy={y} r="4" fill="hsl(158 42% 40%)" opacity={0.4 + Math.cos(i) * 0.3}>
            <animate attributeName="cx" values={`${40-offset};${40+offset};${40-offset}`} dur="3s" repeatCount="indefinite" />
          </circle>
          <line x1={40+offset} y1={y} x2={40-offset} y2={y} stroke="hsl(158 42% 22%)" strokeOpacity="0.2" strokeWidth="1">
